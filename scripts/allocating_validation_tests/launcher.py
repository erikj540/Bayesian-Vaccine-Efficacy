    ...
    for alpha in alphas:
        for vax_prob in vax_probs:
            for prop in sp_proportions:
                ...
                name = f'prev{int(100*prev)}_alpha{int(100*alpha)}_vaxProb{int(vax_prob*100)}_nSpec{n_spec}_nSens{n_sens}_N{N}_se{int(se*100)}_sp{int(sp*100)}_nBurnin{n_burnin}_nSamples{n_samples}_nChains{n_chains}'
